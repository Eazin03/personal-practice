...
# 2.导入数据
message = [
    {"role": "system", "content": "你是金融专家，将文本分类为{}，不清楚的分类为'不清楚'".format(examples_types)}
]

for key, value in examples_data.items():# 在字典后加入items, 将字典的键值对就能直接获取出来
...
for question in questions:
   response = client.chat.completions.create(
       ...
       messages=message + [{"role": "user", "content": f'根据上述实例,来对问题:{question},进行分类,请给出结果'}],  # 输入内容
   )
   # 4.处理结果
