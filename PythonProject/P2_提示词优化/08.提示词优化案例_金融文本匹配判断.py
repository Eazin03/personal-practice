...
{{"role": "user",      "content": f"按照上述示例，回答这2个句子的情况。句子1：[{sentence1}]，句子2：[{sentence2}]"}}
"""
...
for key,vlaue in examples_data.items():
    for i in vlaue:
        message.append(
            {
                "role": "user",
                "content": f"句子1：[{i[0]}]句子2：[{i[1]}]"
            }
        )
        message.append(
            {
...
for question in questions:
    response = client.chat.completions.create(
        ...
        messages=message + [
            {
                "role": "user",
                "content": f"按照上述示例，回答这2个句子的情况。句子1：[{question[0]}]，句子2：[{question[1]}]"
            }
        ]
    )
