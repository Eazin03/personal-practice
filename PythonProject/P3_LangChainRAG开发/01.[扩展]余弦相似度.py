import numpy as np

"""
计算两个向量的余弦相似度(衡量方向相似性,,剔除长度影响)

参数:
    vec_a(np.array): 向量A
    vec_b(np.array): 向量B
返回:
    float: 余弦相似度结果(范围[-1,1], 越接近1方向越一致
公式:
    cos_sim = (vec_a · vec_b) / (||vec_a|| x ||vec_b||)
    拆解:
        1. 点积: vec_a * vec_b = vec_a[0] * vec_b[0] + vec_a[1] * vec_b[1] + ... + vec_a[n] * vec_b[n]
        2. 模长：||vec_a|| = √(vec_a[0]² + vec_a[1]² + ... + vec_a[n]²)
        3. 模长：||vec_b|| = √(vec_b[0]² + vec_b[1]² + ... + vec_b[n]²)

A = [0.5, 0.5]
B = [0.7, 0.7]
C = [0.7, 0.5]
D = [-0.6, -0.5]

"""

# 计算2个向量的点积, 2个向量同维度数字乘积之和
def get_dot(vec_a, vec_b):
    if len(vec_a) != len(vec_b):
       raise ValueError("向量维度不一致") #抛出对应的异常

    dot_sum = 0
    #zip: 同时迭代两个列表的元素,例如: a = [1,2,3], b = [4,5,6], zip(a,b) = [(1,4),(2,5),(3,6)]
    for a, b in zip(vec_a, vec_b):
        dot_sum += a * b

    return dot_sum

# 计算向量的模长
def get_mod(vec):
    # 计算单个向量的模长: 对向量的每个数字求平方, 然后求平方根
    sum_square = 0
    for i in vec:
        sum_square += i ** 2
    #numpy sqrt函数完成开根号
    return np.sqrt(sum_square)


# 余弦相似度计算
def get_cos_sim(vec_a, vec_b):
    dot = get_dot(vec_a, vec_b)
    mod_a = get_mod(vec_a)
    mod_b = get_mod(vec_b)
    return dot / (mod_a * mod_b)

if __name__ == '__main__':
    A = [0.5, 0.5]
    B = [0.7, 0.7]
    C = [0.7, 0.5]
    D = [-0.6, -0.5]
    print("ab:", get_cos_sim(A, B))
    print("ac:", get_cos_sim(A, C))
    print("ad:", get_cos_sim(A, D))
    print("bc:", get_cos_sim(B, C))
    print("bd:", get_cos_sim(B, D))