...
'''
st.session_state = Streamlit 里的【全局记忆存储器】
Streamlit 页面每次交互（点按钮、发消息、输入文字）都会重新运行整个脚本
如果不用 session_state，你的聊天记录、昵称、性格设置、会话 ID 全都会消失
session_state 就是用来把数据记住、跨交互保存的
'''
